...
export const TOOLS = [
  ...
  // ═══════════════════════════════════════════
  {
    ...
    systemPrompt: `You are a commercial real estate LOI assistant for Resolute, Inc., a Colorado-based CRE private equity firm. Given deal inputs (which may be structured fields, freeform text, or extracted from attached documents), produce a complete, professional Sale LOI. Return ONLY valid JSON with these exact keys:
{
...
Return ONLY the JSON. No markdown, no backticks, no explanation.`,
...

  {
    ...
    systemPrompt: `You are a commercial real estate lease LOI assistant for Resolute, Inc. Given lease deal inputs (which may be structured fields, freeform text, or extracted from attached documents), produce a professional Lease LOI. Return ONLY valid JSON:
{
  "loi_date": "(today's date)",
...
Return ONLY the JSON.`,
...

  {
    ...
    systemPrompt: `You are a CRE brokerage agreement assistant for Resolute, Inc. Given deal inputs (which may be structured fields, freeform text, or extracted from attached documents), produce a professional brokerage agreement summary. Return ONLY valid JSON:
{
...
Return ONLY the JSON.`,
  ...
  // ═══════════════════════════════════════════
  {
    ...
    systemPrompt: `You are a CRE tenant representation proposal writer for Resolute, Inc., a Colorado-based commercial real estate firm. Given inputs (which may be structured fields, freeform text, or extracted from attached documents), create a polished, persuasive tenant rep proposal. Return ONLY valid JSON:
{
...
Return ONLY the JSON.`,
...

  {
    ...
    systemPrompt: `You are a CRE broker's email assistant for Resolute, Inc. Given property brochures (which may be attached as PDFs/images or pasted as text) and broker notes, create a polished client-facing email summarizing the options. Extract all relevant property details from the attached brochures — address, size, asking rate, key features, property type, amenities, parking, etc. Every brochure summary should follow the same consistent format. Return ONLY valid JSON:
{
...
Return ONLY the JSON.`,
  ...
  // ═══════════════════════════════════════════
  {
    ...
    systemPrompt: `You are a CRE tour summary writer for Resolute, Inc. Given tour details (which may be structured fields, freeform text, or extracted from attached documents), create a professional tour summary for internal records. Return ONLY valid JSON:
{
...
Return ONLY the JSON.`,
...

  {
    ...
    systemPrompt: `You are a CRE email drafting assistant for Resolute, Inc. Given inputs (which may be structured fields, freeform text, or extracted from attached documents), draft a professional, concise email. Return ONLY valid JSON:
{
...
Return ONLY the JSON.`,
...

  {
    ...
    systemPrompt: `You are a CRM data assistant for Resolute, Inc. Given contact info (which may be structured fields, freeform text, or extracted from attached documents like business cards), produce a clean structured record. Return ONLY valid JSON:
{
...
Return ONLY the JSON.`,
  ...
  // ═══════════════════════════════════════════
  {
    ...
    icon: '💰',
    description: 'Generate a BOV via AutoSheets',
    category: 'Valuation',
    customFlow: 'bov',
    fields: [
      { key: 'senderEmail', label: 'Your Email', type: 'email', placeholder: 'you@resolute.com' },
      { key: 'propertyAddress', label: 'Property Address', type: 'text', placeholder: '4820 S Monaco St, Denver, CO 80237' },
      { key: 'propertyType', label: 'Property Type', type: 'select', options: ['Office', 'Industrial', 'Retail', 'Multifamily', 'Mixed-Use', 'Land', 'Other'] },
      { key: 'squareFootage', label: 'Square Footage', type: 'text', placeholder: '18,500 SF' },
      { key: 'context', label: 'Additional Context', type: 'textarea', placeholder: 'Recent sales comps, cap rate assumptions, current NOI, tenant info, condition notes...' },
    ],
    systemPrompt: '', // Not used — BOV goes through AutoSheets
    pdfTemplate: 'bov',
  },
];
...
// Resolve tool from email subject line (e.g. "SALE LOI" → "sale-loi")
const SUBJECT_MAP = {
  'SALE LOI': 'sale-loi',
  'LEASE LOI': 'lease-loi',
  'BROKERAGE AGREEMENT': 'brokerage',
  'TENANT REP PROPOSAL': 'tenant-rep',
  'BROCHURE SUMMARY': 'brochure',
  'TOUR SUMMARY': 'tour',
  'DRAFT EMAIL': 'email',
  'ADD CONTACT': 'contact',
};

export function resolveToolFromSubject(subject) {
  if (!subject) return null;
  const cleaned = subject.replace(/^(re|fw|fwd):\s*/gi, '').trim().toUpperCase();
  return SUBJECT_MAP[cleaned] || null;
}
